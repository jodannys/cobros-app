...
/*window.renderPanelCartera = function () 
  ...
  <div style="background:#0f172a; background-image:
    ...
    <div style="text-align:center; padding:8px 0 18px">
      <div style="font-size:12px; color:rgba(255,255,255,0.6); font-weight:600;
                  letter-spacing:2px; text-transform:uppercase; margin-bottom:6px">Tienes en mano</div>
    ...
    <div style="display:grid; grid-template-columns:1fr 1fr; gap:8px; margin-bottom:14px">
      ...
      <div style="background:rgba(34,197,94,0.12); border-radius:8px; padding:12px">
        ...
        <div style="font-size:16px; font-weight:800; color:#4ade80">S/ ${(saldo + enMochilas + enLaCalle).toLocaleString('es-PE')}</div>
      </div>
    </div>

    <!-- FÓRMULA -->
    <div style="background:rgba(255,255,255,0.04); border-radius:8px; padding:10px 14px;
                margin-bottom:14px; font-size:11px; color:rgba(255,255,255,0.35);
                text-align:center; line-height:1.8">
      Cartera S/${saldo.toLocaleString('es-PE')}
      &nbsp;+&nbsp;Mochilas S/${enMochilas.toLocaleString('es-PE')}
      &nbsp;+&nbsp;Calle S/${enLaCalle.toLocaleString('es-PE')}
      &nbsp;=&nbsp;
      <span style="color:#93c5fd; font-weight:800">
        S/ ${(saldo + enMochilas + enLaCalle).toLocaleString('es-PE')} total
      </span>
    </div>

    <!-- BOTONES ACCIÓN -->
    <div style="display:grid; grid-template-columns:1fr 1fr; gap:8px;
                margin-bottom:${pendientes.length > 0 ? '10px' : '0'}">
      <button onclick="abrirMovimientoCartera('inyeccion')"
        style="padding:11px 8px; border-radius:8px; border:none; cursor:pointer;
               background:rgba(34,197,94,0.15); color:#4ade80; font-weight:700; font-size:12px">
        ➕ Inyectar Capital
      </button>
      <button onclick="abrirMovimientoCartera('gasto_admin')"
        style="padding:11px 8px; border-radius:8px; border:none; cursor:pointer;
               background:rgba(248,113,113,0.12); color:#f87171; font-weight:700; font-size:12px">
        💸 Gasto Admin
      </button>
      <button onclick="abrirMovimientoCartera('retiro')"
        style="padding:11px 8px; border-radius:8px; border:none; cursor:pointer;
               background:rgba(251,191,36,0.12); color:#fbbf24; font-weight:700; font-size:12px">
        🏠 Retiro Personal
      </button>
      <button onclick="abrirMovimientoCartera('envio_cobrador')"
        style="padding:11px 8px; border-radius:8px; border:none; cursor:pointer;
               background:rgba(96,165,250,0.12); color:#93c5fd; font-weight:700; font-size:12px">
        💰 Enviar a Cobrador
      </button>
    </div>
    ...
    ${pendientes.length > 0 ? `
    <div style="background:rgba(251,191,36,0.1); border-radius:8px; padding:12px 14px; margin-top:10px">
  ...
  <div class="card" style="margin-bottom:12px">
    <div class="card-title">🎒 Mochilas de Cobradores</div>
    ${cobradores.length === 0
